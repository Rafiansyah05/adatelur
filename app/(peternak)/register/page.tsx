...
export default function RegisterPeternakPage() {
  ...
  // State Tahap 2
  const [registrationMethod, setRegistrationMethod] = React.useState<
    'self_form' | 'video_call_cs' | null
  >(null);
  ...
  const handleSignUp = async () => {
    ...
    if (error) {
      const alreadyRegistered =
        /(already registered|already exists|duplicate|Email has already been used|account already exists)/i.test(
          error.message
        );
      if (alreadyRegistered) {
        ...
        if (resend.error) {
          setSignUpError(
            'Email sudah terdaftar. Gagal mengirim ulang verifikasi: ' + resend.error.message
          );
          return;
        }
        setOtpSent(true);
        setAuthMessage(
          'Email sudah terdaftar. Kami mengirim ulang email verifikasi. Silakan cek inbox dan spam.'
        );
        setStep(4);
        return;
      }
      ...
      setLoading(false);
      setSignUpError('Gagal mendaftarkan akun: ' + error.message);
      return;
    }
    ...
    setOtpSent(true);
    setAuthMessage(
      'Email verifikasi telah dikirim. Silakan cek inbox dan spam. Jika Anda menerima link verifikasi, klik link tersebut untuk melanjutkan.'
    );
    setStep(4);
  };
  ...
  const handleVerifyOtpAndSubmit = async () => {
    ...
    try {
      ...
      // 2. Insert Profile, skip or update if already exists
      ...
      if (existingProfileError) {
        throw new Error('Gagal memeriksa profil: ' + existingProfileError.message);
      }
      ...
      } else if (existingProfile.role !== 'peternak') {
        const { error: profileUpdateError } = await supabase
          .from('profiles')
          .update({ role: 'peternak', full_name: nama, phone_number: phone, email })
          .eq('id', userId);
        if (profileUpdateError) {
          throw new Error('Gagal memperbarui profil: ' + profileUpdateError.message);
        }
      }
      ...
      <div className="max-w-md mx-auto">
        ...
        {step === 1 && (
          <Card className="p-6">
            ...
            <div className="space-y-4">
              ...
              <Button onClick={handleNextStep1} className="w-full mt-6">
                Lanjut ke Tahap 2
              </Button>
            </div>
          </Card>
        ...
        {step === 2 && (
          <Card className="p-6">
            ...
            <p className="text-body text-text-desc mb-6">
              Pilih metode pengisian data operasional peternakan Anda.
            </p>
            ...
            {!registrationMethod && (
              <div className="space-y-4">
                <Button
                  variant="secondary"
                  onClick={() => setRegistrationMethod('video_call_cs')}
                  className="w-full py-8 flex flex-col gap-2 h-auto"
                >
                ...
                <Button
                  variant="secondary"
                  onClick={() => setRegistrationMethod('self_form')}
            ...
            {registrationMethod === 'video_call_cs' && (
              <div className="space-y-6 text-center">
                <p className="text-body text-text-main">
                  Silakan hubungi Customer Service kami via WhatsApp untuk dibantu mengisi data
                  operasional.
                </p>
                <a
                ...
                <p className="text-caption text-text-desc">
                  Setelah CS mengonfirmasi, Anda dapat melanjutkan ke Tahap 3.
                </p>
                <div className="flex gap-2">
                  ...
                  <Button onClick={handleNextStep2} className="flex-1">
                    Lanjut Tahap 3
                  </Button>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Input
                    type="number"
                    value={eggProd}
                    onChange={(e) => setEggProd(e.target.value)}
                  />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input
                      type="number"
                      value={eggClean}
                      onChange={(e) => setEggClean(e.target.value)}
                    />
                  </div>
                  <div>
                ...
                {hasVehicle && (
                  <div>
                    ...
                    <Input
                      ...
                      placeholder="Misal: Mobil Pickup, Motor"
                    />
                  </div>
                )}
        ...
        {step === 3 && (
          <div className="space-y-6">
            ...
            <p className="text-body text-text-desc">
              Ambil foto dokumentasi langsung dari kamera perangkat Anda.
            </p>
        ...
        {step === 4 && (
          <Card className="p-6 text-center">
            ...
            <p className="text-body text-text-desc mb-3">
              Kami telah mengirimkan 6 digit kode OTP ke email <strong>{email}</strong>
            </p>
            {authMessage && <p className="text-[14px] text-text-main mb-2">{authMessage}</p>}
            {signUpError && <p className="text-[14px] text-[#E23D28] mb-2">{signUpError}</p>}
            <div className="space-y-4">
              <div>
                <Input
                  type="text"
              ...
              <div className="flex flex-col gap-2">
                <button
                  ...
                  onClick={async () => {
                    ...
                    setLoading(false);
                    if (resend.error) {
                      setSignUpError('Gagal mengirim ulang OTP: ' + resend.error.message);
                    } else {
                      setOtpSent(true);
                      setAuthMessage('Kode OTP baru telah dikirim. Silakan cek email Anda lagi.');
                    }
                  }}
                ...
                {otpSent && <span className="text-[12px] text-text-desc">Kode sudah dikirim</span>}
              </div>
            </div>
          </Card>
        )}
        ...
        {step === 5 && (
          <Card className="p-8 text-center space-y-4">
            ...
            <p className="text-body text-text-desc">
              Data pendaftaran dan foto Anda berhasil dikirim. Proses verifikasi internal
              membutuhkan waktu maksimal 2x24 jam kerja.
            </p>
