...
export default function ConsumerOrdersPage() {
  ...
  const getPeternakName = (order: any) => {
    return order.peternak?.farm_name || order.peternak?.profile?.full_name || 'Peternak Ada Telur';
  };
    ...
    <div className="mx-auto w-full max-w-4xl pb-4 md:pb-8 relative">
      ...
      {displayedOrders.length === 0 ? (
        ...
        <div className="flex flex-col gap-4">
          {displayedOrders.map((order) => {
            const peternakName = getPeternakName(order);
            return (
                ...
                <div className="flex flex-1 items-start gap-4">
                  <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-primary-50 text-lg font-bold text-primary-900 overflow-hidden border border-primary-100">
                    {order.peternak?.profile?.avatar_url ? (
                      <img src={order.peternak.profile.avatar_url} alt={peternakName} className="h-full w-full object-cover" />
                    ) : (
                      <span>{peternakName[0].toUpperCase()}</span>
                    )}
                  </div>
                  ...
                  <div className="flex-1">
                      ...
                      <span
                        className={`px-2 py-0.5 rounded text-xs font-bold ${
                          order.order_status === 'completed'
                            ...
                            : order.order_status === 'rejected' || order.order_status === 'expired'
                    ...
                    <p className="text-body-medium text-text-main mb-2">
                      {peternakName} • {order.rak_quantity} Rak
                    </p>
          ...
          <div className="relative bg-white w-[90%] max-h-[80vh] md:w-[420px] md:h-full md:max-h-none rounded-xl md:rounded-none overflow-y-auto p-6 shadow-xl animate-in slide-in-from-bottom-4 md:slide-in-from-right-4 duration-300">
            ...
            <div className="relative border-l-2 border-neutral-200 ml-3 space-y-8">
              <div className="relative pl-6">
                <div
                  className={`absolute -left-[9px] top-1 h-4 w-4 rounded-full border-2 bg-white transition-colors duration-300 ${
              ...
              <div className="relative pl-6">
                <div
                  className={`absolute -left-[9px] top-1 h-4 w-4 rounded-full border-2 bg-white transition-colors duration-300 ${
                    trackingOrder.order_status === 'completed' ? 'border-primary-500 bg-primary-500' : 'border-neutral-300'
                  }`}
                ></div>
                <h4
                  className={`font-bold transition-colors duration-300 ${
            ...
            {trackingOrder.order_status === 'completed' && trackingOrder.delivery_proof?.photo_url && (
              <div className="mt-8 border-t border-neutral-200 pt-6">
                ...
                <div className="bg-neutral-100 rounded-lg overflow-hidden relative h-48 w-full border border-neutral-200">
                  <Image src={trackingOrder.delivery_proof.photo_url} alt="Bukti Pengiriman" fill className="object-contain" />
                </div>
      ...
      {proofData && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/90 p-4">
          <button onClick={() => setProofData(null)} className="absolute top-4 right-4 p-2 bg-white/10 hover:bg-white/20 rounded-full text-white transition-colors">
            <X size={24} />
          </button>
          <div className="relative w-full max-w-2xl bg-white rounded-lg overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="relative w-full aspect-square md:aspect-video bg-black">
              <Image src={proofData.photo_url} alt="Bukti Pengiriman" fill className="object-contain" />
            </div>
            {(() => {
      ...
      <RatingModal
        ...
        onSuccess={() => {
          setOrders((prev) => prev.map((o) => (o.id === ratingOrderId ? { ...o, rating: 5 } : o)));
        }}
      />
    </div>
  );
